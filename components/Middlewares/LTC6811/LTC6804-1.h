...
0x585a, 0x8ba7, 0x4e3e, 0x450c, 0x8095};
 ...
 /*!
  ...
  |11| 3    | Filtered 		   	  |
*/
#define MD_FAST 1              //快速模式   27KHz
#define MD_NORMAL 2            //标准模式   7KHz
#define MD_FILTERED 3          //滤波模式   26KHz
...
//uint8_t CHG = 0; //!< aux channels to be converted 要转换的辅助通道
 /*!****************************************************
...
********************************************************/
#define DCP_DISABLED 0        //不允许放电
#define DCP_ENABLED 1         //允许放电
...
void set_adc(uint8_t MD,uint8_t DCP,uint8_t CH,uint8_t CHG);//设置ADC状态

void LTC6804_adcv(void); //启动电池电压ADC转换和轮询状态

void LTC6804_adax(void);//启动GPIO ADC转换和轮询状态

uint8_t LTC6804_rdcv(uint8_t reg, uint8_t total_ic, uint16_t cell_codes[][12]);//读取电池电压

void LTC6804_rdcv_reg(uint8_t reg, uint8_t nIC, uint8_t *data);

int8_t LTC6804_rdaux(uint8_t reg, uint8_t nIC, uint16_t aux_codes[][6]);//读取辅助寄存器

void LTC6804_rdaux_reg(uint8_t reg, uint8_t nIC,uint8_t *data);

void LTC6804_clrcell(void); //清除电池电压寄存器A、B、C 和D。

void LTC6804_clraux(void);//清除辅助寄存器 A 和 B

void LTC6804_wrcfg(uint8_t nIC,uint8_t config[][6]);//写配置寄存器

int8_t LTC6804_rdcfg(uint8_t nIC, uint8_t r_config[][8]);//读配置寄存器

void delayMicroseconds(unsigned int x);

void wakeup_idle(void);//括号后需加void，否则报警
...
void LTC6804_cvst(void);//自测试模式

void LTC6804_ADSTAT(void);//启动状态组ADC转换，滤波模式

int8_t LTC6804_rdstat(uint8_t reg,uint8_t total_ic,uint16_t stat_codes[][6]);//读状态寄存器

void LTC6804_rdstat_reg(uint8_t reg,uint8_t total_ic,uint8_t *data);

void LTC6804_adcvax(void); //启动组合电池电压以及GPIO1、GPIO2转换和轮询状态

//void Write_Balance_Commond(void);  //均衡命令
...
void stcomm_config(uint8_t total_ic);
